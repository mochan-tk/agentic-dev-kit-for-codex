...
def history_bytes(**binding):
    """Load reviewed old bytes without an old object store or network."""
    spec = importlib.util.spec_from_file_location("product_fixtures", ROOT / ".github/scripts/check-product.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.history_bytes(ROOT, **binding)


class GovernanceProcedureProvenanceTests(unittest.TestCase):
    def test_bootstrap_is_mandatory_and_engine_remains_byte_identical(self):
        self.assertEqual(0, self.cli(ROOT).returncode)
        for name in ("scaffold-init.sh", "scaffold-init.ps1", "scaffold-install.sh"):
            root = self.fixture()
            (root / ".github/scripts" / name).unlink()
            self.assertNotEqual(0, self.cli(root).returncode)
        root = self.fixture()
        engine = root / ".github/scripts/scaffold-install.sh"
        ...
        record["bootstrap"]["target_files"][-1]["sha256"] = hashlib.sha256(engine.read_bytes()).hexdigest()
        parity.write_text(json.dumps(record))
        self.assertNotEqual(0, self.cli(root).returncode)

    def test_bootstrap_regular_permission_variants_keep_git_nonexecutable_class(self):
        root = self.fixture()
        for mode in (0o600, 0o640, 0o644, 0o664):
            for name in ("scaffold-init.sh", "scaffold-init.ps1", "scaffold-install.sh"):
                (root / ".github/scripts" / name).chmod(mode)
            self.assertEqual(0, self.cli(root).returncode)
        for mode in (0o755, 0o4644):
            (root / ".github/scripts/scaffold-install.sh").chmod(mode)
            self.assertNotEqual(0, self.cli(root).returncode)

    def fixture(self):
        temporary = tempfile.TemporaryDirectory(prefix="procedure-provenance-")
    ...
    def test_production_cli_requires_closed_companion_and_exact_targets(self):
        self.assertEqual(0, self.cli(ROOT).returncode)
        for mutation in (lambda r: r.pop("governance_procedures"),
                         lambda r: r["governance_procedures"].update(extra="unreviewed"),
                         lambda r: r["governance_procedures"].update(integration="installed-by-default"),
                         lambda r: r["governance_procedures"]["target_files"].pop(),
                         lambda r: r["governance_procedures"]["source_files"].update({".github/scripts/governance-status.sh":"0"*40})):
            root=self.fixture(); path=root/".github/distribution/source-parity.v1.json"
            record=json.loads(path.read_text());mutation(record);path.write_text(json.dumps(record))
            self.assertNotEqual(0,self.cli(root).returncode)
        for action in ("missing", "symlink", "executable", "special", "content"):
            with self.subTest(action=action):
                root=self.fixture();path=root/".github/scripts/governance-status.sh"
                if action=="missing":path.unlink()
                elif action=="symlink":path.unlink();path.symlink_to(root/"README.md")
                elif action=="executable":path.chmod(0o755)
                elif action=="special":path.chmod(0o4644)
                else:path.write_text(path.read_text()+"\n# unbound change\n")
                self.assertNotEqual(0,self.cli(root).returncode)

    def test_new_companion_accepts_ordinary_read_write_mode_variation(self):
        root=self.fixture()
        for mode in (0o600,0o640,0o644,0o664):
            (root/".github/scripts/governance-status.sh").chmod(mode)
            self.assertEqual(0,self.cli(root).returncode)
...
""")
    ...
    def test_f2_bootstrap_retains_exact_historical_anchor(self):
        old = history_bytes(blob="f66d3aa5e73abf24052c70f557cd6df9177ca012")
        base, head = self.adoption_fixture(anchor=old)
        result = self.ritual(base, head)
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)

    def test_f2_bootstrap_retains_t18_anchor(self):
        old = history_bytes(blob="cc888c829bc5957871376004cb59a93b4980b50f")
        base, head = self.adoption_fixture(anchor=old)
        result = self.ritual(base, head)
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
    ...
    def clone_source(self):
        ...
        (source / ".github/scripts").mkdir()
        for name in ("scaffold-init.sh", "scaffold-init.ps1", "scaffold-install.sh"):
            shutil.copyfile(ROOT / ".github/scripts" / name, source / ".github/scripts" / name)
        return source
...
""")
    ...
    def test_audit_ruleset_help_examples_are_executable_previews(self):
        ...
        for example in examples:
            with self.subTest(example=example):
                ...
                # Evidence-reading team/reconcile previews are executed from
                # the installed Skill/help by the raw stateful governance driver.
                if "--reconcile" in args or "team" in args:
                    continue
    ...
    def assert_payload_upgrade_preserves_adopter_state(self, paths, base, *, changed=None):
        ...
        for name in paths:
            data = history_bytes(commit=base, path=PAYLOAD + "/" + name)
