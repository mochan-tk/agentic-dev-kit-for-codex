...
CURRENT_TASK_ID = "T26"
CURRENT_TASK_BRANCH = "codex/source-first-completion-contract"
EXPECTED_I02 = (
...
class RepositoryPolicyTest(unittest.TestCase):
    ...
    def test_audit_deep_base_and_accepted_json_are_classified(self):
        ...
        with mock.patch.object(self.checker, "git_blob_bytes", return_value=malformed):
            ...
            self.assertEqual(["accepted Task T26 transition evidence base ownership manifest is invalid"], errors)
    ...
    def test_phase2_frontier_is_exact(self):
        ...
        self.assertEqual(["T26"], [task["id"] for task in active])
        self.assertEqual(
            self.checker.EXPECTED_T26_PATHS,
            tuple(entry["path"] for entry in active[0]["owned_paths"]),
        )
        self.assertEqual(9, len(active[0]["owned_paths"]))
        self.assertEqual('https://github.com/mochan-tk/agentic-dev-kit-for-codex/issues/45', active[0]['record'])
        self.assertEqual('4f34ffff13e82ca06b397fec4dae34483c9d9711', active[0]['base_commit'])
        self.assertEqual('00a8a6eaa4fcc4ad1970f136ccd811a3b86e6fe6', active[0]['base_tree'])
        t25 = next(task for task in payload['tasks'] if task['id'] == 'T25')
        self.assertEqual('accepted', t25['state'])
        self.assertEqual(12, len(t25['owned_paths']))
        self.assertEqual(tuple(path for path in self.checker.EXPECTED_T25_PATHS
                               if path not in self.checker.EXPECTED_T26_PATHS),
                         tuple(entry['path'] for entry in t25['owned_paths']))
        t24 = next(task for task in payload['tasks'] if task['id'] == 'T24')
        self.assertEqual('accepted', t24['state'])
        self.assertEqual(3, len(t24['owned_paths']))
        self.assertEqual(tuple(path for path in self.checker.EXPECTED_T24_PATHS
                               if path not in self.checker.EXPECTED_T25_PATHS),
        ...
        self.assertEqual(2, len(t19['owned_paths']))
        ...
        self.assertEqual(tuple(path for path in self.checker.EXPECTED_T18_PATHS
                               if path not in self.checker.EXPECTED_T19_PATHS
                               and path not in self.checker.EXPECTED_T22_PATHS
                               and path not in self.checker.EXPECTED_T24_PATHS),
                         tuple(entry['path'] for entry in t18['owned_paths']))
    ...
    def test_manifest_rejects_a_second_active_task(self):
        ...
        phase0["state"] = "active"
        self.assertEqual("T26", self.active_task(payload)["id"])
        errors = []
        self.checker.validate_manifest(payload, errors)
        self.assert_rejected(errors, "exactly one active Task")

    def test_t26_transition_rejects_binding_scope_and_accepted_owner_drift(self):
        for task_id, field, value in (
            ('T26', 'record', 'https://github.com/mochan-tk/agentic-dev-kit-for-codex/issues/43'),
            ('T26', 'branch', 'codex/unreviewed'), ('T26', 'base_commit', '0' * 40),
            ('T26', 'base_tree', '0' * 40), ('T26', 'state', 'accepted'),
            ('T26', 'path_transitions', [{'operation': 'delete'}]), ('T26', 'owned_paths', []),
            ('T25', 'record', 'https://github.com/mochan-tk/agentic-dev-kit-for-codex/issues/41'),
            ('T25', 'branch', 'codex/unreviewed'), ('T25', 'base_commit', '0' * 40),
            ('T25', 'base_tree', '0' * 40), ('T25', 'state', 'active'),
            ('T25', 'path_transitions', [{'operation': 'delete'}]), ('T25', 'owned_paths', []),
            ('T24', 'record', 'https://github.com/mochan-tk/agentic-dev-kit-for-codex/issues/39'),
            ('T24', 'branch', 'codex/unreviewed'), ('T24', 'base_commit', '0' * 40),
            ('T24', 'base_tree', '0' * 40), ('T24', 'state', 'active'),
            ('T24', 'path_transitions', [{'operation': 'delete'}]), ('T24', 'owned_paths', []),
            ...
            ('T23', 'base_tree', '0' * 40), ('T23', 'state', 'active'),
            ...
            ('T21', 'base_tree', '0' * 40), ('T21', 'state', 'active'),
        ...
        errors = []
        self.checker.validate_phase2_frontier(payload, errors)
        self.assert_rejected(errors, 'T26 paths must all use mode 100644')
        ...
        for operation, destination in cases:
            with self.subTest(operation=operation):
                ...
                if destination is not None:
                    ...
                    # the real T26 manifest must continue rejecting this expansion.
