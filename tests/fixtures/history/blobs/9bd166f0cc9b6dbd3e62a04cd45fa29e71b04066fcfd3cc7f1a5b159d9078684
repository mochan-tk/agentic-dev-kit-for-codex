...
adoption, and (b) proposes explicit owner review for `specs/**` so future spec changes
get owner review (if CODEOWNERS is selected, it applies from the base branch, so the rule
