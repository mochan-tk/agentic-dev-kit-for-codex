...
> Historical source rationale, not an accepted adopter decision. Source Issue
> numbers and activation gates below belong to the frozen Copilot repository.
> The installed AGENTS.md and Codex Skills supersede platform-specific gates;
> adopt any stacked-work policy only through the adopter's own Task agreement.

- **Source status:** accepted in the source repository; adopter acceptance not implied
- **Date:** 2026-08-09
...
- The recovery mechanism (Resume protocol,
  `.agents/skills/session-orchestration/SKILL.md`) restores a *dead* session
...
Meanwhile the sessions runtime (source app, and any orchestrator
...
3. **Stacked PRs are the multi-worker case — shape defined now,
   ...
   layer's branch. Mechanics: worker branches `codex/task-<n>-<slug>-p<k>` (or
...
- Update `.agents/skills/session-orchestration/SKILL.md`: mapping table
  gains the worker tier; parent protocol and report formats gain the extra
  hop; kickoff template for workers.
- Update `.agents/skills/task-routing/SKILL.md`: routing applies per
...
- `.agents/skills/session-orchestration/SKILL.md` — current mapping and
  protocols.
...
## Codex adaptation and evidence boundary

The two-tier supervisor/worker responsibility is retained, not the source
app's mandatory Project-parent topology. Epic threads are siblings;
Project coordination is one responsibility with an optional sibling UI.
Use only an available documented Codex surface or an explicit manual handoff.
The shipped role definitions and ritual sensor are guidance/validation, not
authenticated identities, automatic CI enforcement or proof of runtime parity.
Source issue numbers in the rationale describe source history, not adopter
acceptance; no development Task, pin or qualification status is installed.
