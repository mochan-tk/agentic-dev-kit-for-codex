...
- `.github/docs/context/` is an **intake area**: raw, possibly redundant, never
  ...
  `.agents/skills/context-collection/SKILL.md`. Do not "clean up" raw material
...
- `.github/docs/agreements/` is **reviewed truth**: requirements (`REQ-###`), ADRs
  ...
  bar belongs here (`.agents/skills/context-distillation/SKILL.md`, "When an
...
- **Wording fixes** — typos, broken links, formatting, phrasing that changes
  ...
  approval from the declared adopter agreement owner before merge; use
  CODEOWNERS only when the adopter has explicitly configured it.
