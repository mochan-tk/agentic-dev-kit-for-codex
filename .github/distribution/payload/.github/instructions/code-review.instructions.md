...
These standards guide in-loop advisory critique critique, available code review,
agent reviewers (`.codex/agents/reviewer.toml`), and human reviewers. A
...
|---|---|---|
| **advisory critique** (supported CLI/app surfaces) | In-loop critique of plans, designs, implementations, and tests; contrasting-model blind spots | Final Task audit, formal PR approval, or unsupported surfaces |
| **available code review** | Generic bugs, vulnerabilities, logic defects, and craft in a diff or PR | Task evidence/ownership completeness or formal approval; its result is advisory |
...
Codex review tools are optional and surface-specific; verify availability.
No service or reviewer identity is authenticated by these files.

advisory critique runs inside the work loop, before a PR audit, and is therefore not
...
1. **Evidence (custom reviewer + human).** The PR references its Task issue
   (`Closes #<n>`, or `Refs #<n>` for post-merge acceptance) and the
...
6. **Code defects (available code review + human).** Hunt for substantive bugs,
   vulnerabilities, logic errors, and performance failures beyond check 3's
   test-integrity obligation. advisory critique should already have challenged
   these in-loop on supported surfaces. AI findings are advisory; they never
   satisfy required approval.
7. **Craft (available code review + human).** Treat naming, structure,
