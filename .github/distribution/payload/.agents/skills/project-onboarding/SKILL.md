...
2. Reuse the adopter's explicitly reviewed `solo` or `team` profile, or ask
   ...
   - **Create disabled, review later** — request `disabled` enforcement.
     ...
     (`--reconcile --enforcement active`), or a reviewed Settings change.
...
helper verifies absence/canonical content before writes; unknown or customized
configuration refuses. Do not auto-retry a refusal with a guessed profile or
reconciliation choice. Team intent also requires complete issuer evidence and
resolved installed CODEOWNERS; selecting team does not authorize creating it.

<!-- onboarding-ruleset-apply -->
```bash
case "${ONBOARD_CHOICE:?record the consent choice}" in
  skip) printf '%s\n' 'Branch protection skipped; record how to resume.' ;;
  active|disabled)
    case "${ONBOARD_RULESET:?review new or canonical existing ruleset}" in
      new) set -- ;;
      canonical) set -- --reconcile ;;
      *) printf '%s\n' 'Unknown ruleset selection; no setup attempted.' >&2; exit 2 ;;
    esac
