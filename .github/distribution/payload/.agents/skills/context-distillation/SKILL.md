...
4. **Write the diff, open a PR.** Agreements change only via PR with human
   ...
   `REQ-###`/ADR with its source link. **Owner-reviewed merge = agreement**; the review
...
|---|---|---|---|
| Always-on | Needed on *most* tasks, stable, compressible to a few lines | `AGENTS.md`, `.github/codex-instructions.md` | keep each file lean — target well under ~150 lines; adding usually means removing |
| Scoped | Needed only when touching certain paths | explicitly referenced `.github/instructions/<area>.instructions.md` (Codex does not automatically apply these globs) | one concern per file |
| On-demand | Large, procedural, or rarely needed | a skill under `.agents/skills/` or a doc under `.github/docs/agreements/` referenced from issues | no practical size limit; discoverability via skill descriptions and issue links |
